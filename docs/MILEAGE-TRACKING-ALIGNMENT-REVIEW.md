...
1. **Missing Journey Tracking Table** ❌
   - **Issue**: No table with `journey_id` as PK exists for journey-level tracking
   - **Current**: Only `Trips` table with `TripID` exists (for expense reporting, different purpose)
   - **Fix**: Created migration script `scripts/create-trips-table.sql` for `travel_journeys` table
   - **Note**: Renamed to `travel_journeys` to avoid SQL Server case-insensitivity conflict with existing `Trips` table
