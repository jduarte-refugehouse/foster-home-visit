...
### Database Migration
- `scripts/create-trips-table.sql` - Creates `travel_journeys` table with `journey_id` as PK
  - **NOTE**: Renamed from `trips` to `travel_journeys` to avoid conflict with existing `Trips` table (with `TripID` PK) used for expense reporting
...
2. **Run Database Migration**
   ...
   - This creates the `travel_journeys` table (separate from existing `Trips` table)
